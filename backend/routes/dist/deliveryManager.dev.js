...
router.post('/getPersonId', function (req, res, next) {
  sql = 'select person.person_id as id\
    ...
    OR person.person_phone like ? AND person.person_email LIKE ? AND person.person_lname LIKE ?'; //     {name: "fname", value: "Math"}
  // 1: {name: "lname", value: ""}
  // 2: {name: "email", value: ""}
  // 3: {name: "phone", value: ""}

  var personDetails = [req.body.email, req.body.phone, req.body.email, req.body.fname, req.body.phone, req.body.email, req.body.lname]; // res.send("Got here!")
...
router.post('/getAddress', function (req, res, next) {
  ...
  var addressDetails = [req.body.streetNum, req.body.streetName, req.body.suburb, req.body.city, req.body.postcode];
...
function findAddress(req, success) {
  ...
  if (req.body.address_id == -1) {
    sql = "INSERT INTO `address` (`add_id`, `add_num`, `add_street`, `add_suburb`, `add_city`, `add_postcode`) VALUES (NULL, ?, ?, ?, ?, ?)"; //     {name: "fname", value: "Math"}
    // 1: {name: "lname", value: ""}
    // 2: {name: "email", value: ""}
    // 3: {name: "phone", value: ""}

    var addressDetails = [req.body.streetNum, req.body.streetName, req.body.suburb, req.body.city, req.body.postcode]; // res.send("Got here!")
...
function findPerson(req, add_id, success) {
  ...
  if (req.body.person_id == -1) {
    sql = "INSERT INTO `person` (`person_id`, `person_phone`, `person_email`, `person_fname`, `person_lname`, `person_pass`, `person_user_level`, `add_id`) VALUES (NULL, ?, ?, ?, ?, 'P@ssword', '0', ?)"; //     {name: "fname", value: "Math"}
    // 1: {name: "lname", value: ""}
    // 2: {name: "email", value: ""}
    // 3: {name: "phone", value: ""}

    var addressDetails = [req.body.phone, req.body.email, req.body.fname, req.body.lname, add_id]; // res.send("Got here!")
    ...
    sql = "UPDATE `person` SET `person_phone` = ?, `person_email` = ?, `person_fname` = ?,\
     `person_lname` = ? ,  `add_id` = ? WHERE `person`.`person_id` = ?"; //     {name: "fname", value: "Math"}
    // 1: {name: "lname", value: ""}
    // 2: {name: "email", value: ""}
    // 3: {name: "phone", value: ""}

    var _addressDetails = [req.body.phone, req.body.email, req.body.fname, req.body.lname, add_id, person_ID]; // res.send("Got here!")

    con.query(sql, _addressDetails, function (err, result) {
      if (err) throw err;
...
router.post('/submitRecipient', function (req, res, next) {
  try {
    findAddress(req, function (add_id) {
      findPerson(req, add_id, function (person_id) {
        ...
        con.query(sql, [person_id], function (err, result) {
          ...
          if (result.length > 0) {
            ...
            referrerDetails = [req.body.dogs, req.body.child_under_5, req.body.child_between_5_10, req.body.child_between_11_17, req.body.adults, req.body.dietaryReq, req.body.allergyNotes, person_id];
          } else {
            ...
            `rec_children_between_11_17`, `rec_adults`, `rec_dietary_req`, `rec_allergies`)\
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            referrerDetails = [person_id, req.body.dogs, req.body.child_under_5, req.body.child_between_5_10, req.body.child_between_11_17, req.body.adults, req.body.dietaryReq, req.body.allergyNotes];
          } // res.send("Got here!")
      ...
      findPerson(req, add_id, function (person_id) {
        ...
        con.query(sql, [person_id], function (err, result) {
          ...
          if (result.length > 0) {
            ...
            referrerDetails = [req.body.RefType, req.body.refNotes, req.body.refOrg, person_id];
          } else {
            //if the recipient doesnt exist insert
            sql = "INSERT INTO `referrer` (`person_id`, `RT_type`, `notes`, `organisation`) VALUES (?, ?, ?, ?)";
            referrerDetails = [person_id, req.body.RefType, req.body.refNotes, req.body.refOrg];
          } // res.send("Got here!")
...
router.post('/submitDelivery', function (req, res, next) {
  ...
  if (req.body.ref !== '') {
    ...
    sqlData = [req.body.ref, req.body.rec, req.body.branch];
  } else {
    sql = "INSERT INTO `delivery` (`delivery_id`, `vol_id`, `ref_id`, `recipient_id`, `delivery_status`, \
    `delivery_est_time`, `delivery_start`, `delivery_end`, `branch_id`)\
    VALUES (NULL, NULL, NULL, ?, '1', 'current_timestamp()', NULL, NULL, ?)";
    sqlData = [req.body.rec, req.body.branch];
  }
