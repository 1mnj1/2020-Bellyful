...
router.post('/getFreezerManager', function(req, res, next) {
  //sql query for the data
  sql = "select CONCAT(person.person_fname , ' ' , person.person_lname) as 'Name' ,CONCAT(address.add_num , ' ' , address.add_street) as 'Address', branch.branch_name as Branch\
  from freezer,person , address, branch\
